...
type UserChannel = {
  ...
  queue: string[];
  resolve?: undefined | ((value: IteratorResult<string>) => void);
  closed: boolean;
};
...
export default class TelegramBot {
  ...
  private batchProcessor = new ThrottledBatchProcessor<number>(chatIds => this.flushAll(chatIds), 250);
  ...
  private async handleMessage(msg: Message): Promise<void> {
    ...
    if (chatType === "group" || chatType === "supergroup") {
      ...
      const agent = this.ensureAgentForChat(chatId, groupConfig.agentType);
    ...
    if (chatType === "private") {
      ...
      const agent = this.ensureAgentForChat(fromUserId, this.botConfig.dmAgentType);
      ...
      if (document.mime_type?.startsWith("image/")) {
        ...
        if (document.file_size && document.file_size > this.botConfig.maxDocumentSize) {
          ...
          try {
            ...
            const mimeType = ChatAttachmentSchema.shape.mimeType.parse(document.mime_type);
  ...
  private ensureAgentForChat(chatId: number, agentType: string): Agent {
  ...
  private async sendMessageWithFallback(chatId: number, text: string): Promise<Message> {
    ...
    this.app.serviceOutput(this.telegramService, `Text sent, messageId=${message.message_id}`);
    return message;
  }
