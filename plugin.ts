...
import {TelegramServiceConfigSchema, type ParsedTelegramBotConfig} from "./schema.ts";
...
function addBotsFromEnv(bots: Record<string, Partial<ParsedTelegramBotConfig>>) {
  for (const [key, value] of Object.entries(process.env)) {
    ...
    bots[name] = {
      ...
      botToken: value,
      dmAgentType,
      escalation: process.env[`TELEGRAM_ESCALATION_GROUP${n}`]
        ? {group: process.env[`TELEGRAM_ESCALATION_GROUP${n}`]!}
        : undefined,
      groups: {},
    };
  }
  ...
  install(app, config) {
    ...
    app.waitForService(EscalationService, escalationService => {
      for (const [botName, bot] of Object.entries(config.telegram.bots)) {
        if (bot.escalation) {
          escalationService.registerProvider(botName, new TelegramEscalationProvider({type: "telegram", bot: botName, group: bot.escalation.group}));
        }
      }
    });
  },
